...
P = {
    ...
    "melon_tiles": 10,            # opening melon tiles (day 0-1)
    "melon_repeat": 10,           # extra melon tiles to keep replanting mid-game (0 = opening only)
    "melon_last_day": 14,         # latest day a melon may be planted (needs 10 days to mature)
    "melon_guard": 90,            # stop planting melons when the marginal melon price is below this
    "weed_dig_value": 12.0,       # priority of clearing a weed
    "wheat_fill_max": 999,        # cap on filler wheat tiles
    ...
    "wheat_floor": 0,
    "fill_crops": ["WHEAT"],
    "endgame_crop": "VALUE",     # "CARROT" floods carrots in the last 6 days; "VALUE" lets fill_value choose
    "endgame_fill_crops": ["WHEAT", "CARROT"],        # min value per tile-day for filler crops (below: leave tile empty)             # fertilize wheat/carrot only below this fertilizer price
...
def recipe_targets(ctx, counts):
    ...
    if d <= 1:
        T["MELON"] = P["melon_tiles"]
    elif P["melon_repeat"] > 0 and d <= P["melon_last_day"] and ctx.days_left >= 11:
        T["MELON"] = P["melon_repeat"]
    else:
        T["MELON"] = 0
    T["WHEAT"] = 999  # fill
    if ctx.days_left <= 6:
        T["CARROT"] = (999 if ctx.days_left >= 4 else 0) if P["endgame_crop"] == "CARROT" else 0
    return T, full
...
def fill_tiles(ctx, mem, econ, plan, free, counts, buy, budget, labor_left, T):
    ...
    while free and labor_left >= P["load_plant"]:
        best = None
        n_wheat = counts.get("WHEAT", 0)
        crops = P["endgame_fill_crops"] if ctx.days_left <= 6 else P["fill_crops"]
        for name in crops:
            if name == "WHEAT" and n_wheat >= P["wheat_fill_max"]:
                continue
...
def plan_day(ctx, mem):
    ...
    buy = {}
    n_new = 0
    # capital discipline: keep a slice of cash for cheap crop tiles so land does not sit idle
    fill_hold = 0.0
    if P["fill_reserve"] > 0 and ctx.days_left >= 8:
        idle = len(free)
        fill_hold = min(float(P["fill_reserve"]), idle * 100.0)
    hours_ok = ctx.hours_left >= 4 and not ctx.last_day and ctx.days_left >= 3
    guard = P["guard_price"]
    order = list(P["alloc_order"])
    if ctx.days_left <= 6:
        order = ["CARROT", "FILL"] if P["endgame_crop"] == "CARROT" else ["FILL"]
    for name in order:
        ...
        while counts[name] < target and free:
            avail = budget - (fill_hold if name in ANIMALS else 0.0)
            if cost > avail:
                break
            ...
            if product in guard and units > 0:
                ...
                thresh = P["melon_guard"] if name == "MELON" else guard[product]
                if mp < thresh:
                    break
...
def gen_tasks(ctx, mem, plan):
    ...
    for x, y in ctx.weeds:
        if (x, y) not in plan:
            add((x, y), ["DIG"], P["weed_dig_value"] if ctx.days_left > 4 else 0.0)
    for x, y, t in ctx.structures:
