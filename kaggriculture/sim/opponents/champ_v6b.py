...
def recipe_targets(ctx, counts):
    ...
    sheep_cap = int(max(2, min(P["max_sheep"], round(2 + exp["WOOL"] / P["sheep_k"]))))
    T["SHEEP"] = min(sheep_cap, 2 + d // 2) if d <= P["sheep_last_day"] else 0
    T["STRAWBERRY"] = int(max(8, min(P["straw_cap"], round(P["straw_base"] + P["straw_k"] * exp["STRAWBERRY"])))) if d <= P["straw_last_day"] else 0
    T["TOMATO"] = int(round(exp["TOMATO"] * 0.8)) if (exp["TOMATO"] >= P["tomato_min_dem"] and d <= P["tomato_last_day"]) else 0
...
def plan_day(ctx, mem):
    ...
    guard = P["guard_price"]
    order = ["COW", "SHEEP", "MELON", "STRAWBERRY", "TOMATO", "WHEAT", "CARROT"]
    if ctx.days_left <= 6:
        order = ["CARROT", "WHEAT"]  # only fast crops late
        T["CARROT"] = 999 if ctx.days_left >= 4 else 0
        T["WHEAT"] = 999 if ctx.days_left >= 5 else 0
    for name in order:
        ...
        while counts[name] < target and free:
            ...
            units = {"COW": 25.0, "SHEEP": 20.0, "STRAWBERRY": 8.0, "TOMATO": 7.0, "MELON": 6.0}.get(name, 0.0)
            if name in guard and units > 0:
                mp = econ.marginal(product, units, lump=(name in ("MELON",)))
                if mp < guard[name]:
                    break
